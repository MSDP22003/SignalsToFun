...
# make sure each task has its own variables - no shared variables between threads
# lock variables when they're being used
...
def exampleJob(worker):
    ...
    with print_lock:
        print(threading.current_thread().name, worker) #will print current thread name & worker
...
for x in range(10):  #allows for 10 threads
    t = threading.Thread(target = threader)  #defining threads and target of threads
    t.daemon = True  #daemon means it will die when the main thread dies, have to have this to define it
    t.start()  #start threading after defining it

start = time.time()

for worker in range(20):  #20 instances
    q.put(worker)

q.join() #waits until thread terminates

print('Entire job took:', time.time() - start)


## Everything below here is a comment on the youtube video that  I just watched with further explanations:
